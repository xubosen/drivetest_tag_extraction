...
class Question(BaseModel):
    ...
    def validate_string_lists(cls, v: List[str]) -> List[str]:
        ...
        for item in v:
            ...
            if len(stripped_item) < 1:
                raise ValueError(
                    "All tags and keywords must be non-empty strings")
    ...
    def set_tags(self, tags: List[str]):
        ...
        if not all(len(tag) >= 1 for tag in tags):
            raise ValueError("All tags must be at least 1 character long")
        self.tags = tags.copy()
    ...
    def set_keywords(self, keywords: List[str]):
        ...
        if not all(len(keyword) >= 1 for keyword in keywords):
            raise ValueError("All keywords must be at least 1 character long")
        self.keywords = keywords.copy()

    def add_keyword(self, keyword: str):
        """Add a single keyword to the question's existing keywords."""
        if len(keyword) < 1:
            raise ValueError("Keyword must be at least 1 character long")
